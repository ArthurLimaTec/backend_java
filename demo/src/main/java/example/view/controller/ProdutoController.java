package example.view.controller;
...
import example.services.ProdutoService;
import example.shared.ProdutoDTO;
import example.view.model.ProdutoRequest;
import example.view.model.ProdutoResponse;
...
public class ProdutoController {
    ...
    public ResponseEntity<List<ProdutoResponse>> obterTodos(){
        List<ProdutoDTO> a =  produtoService.obterTodos();
        
        ModelMapper mapper = new ModelMapper();

        List<ProdutoResponse> resposta = a.stream().map(produtoDto -> mapper.map(produtoDto, ProdutoResponse.class))
